...
if __name__ == '__main__':
    ...
    if size != 2:
        if rank == 0:
            print("This code requires exactly 2 MPI processes.")
        sys.exit(1)
    ...
    if rank == 1: 
        ...
        grid = Grille(*init_pattern)  # Crear la cuadrícula en el proceso 1
        comm.send(grid.cells, dest=0)  # Enviar la matriz de celdas al proceso 0
        ...
        cells = comm.recv(source=1)  # Recibir la matriz de celdas del proceso 1
        grid = Grille(cells.shape, None, pg.Color("black"), pg.Color("white"))  # Crear la cuadrícula con las celdas recibidas
        appli = App((resx, resy), grid)  # Crear la aplicación con la cuadrícula recibida

        while True:
            cells = comm.recv(source=1)  # Recibir la matriz de celdas del proceso 1
            grid.cells = cells  # Actualizar la matriz de celdas de la cuadrícula local
            print('cells', cells)
            appli.draw()  # Dibujar la cuadrícula
