...
if __name__ == '__main__':
    ...
    dico_patterns = { # Dimension et pattern dans un tuple
        ...
        "beacon2"  : ((12,12), [(4,6),(4,7),(5,6),(5,7),(3,4),(6,5),(7,4),(7,5)]),
        ...
        "floraison3" : ((16,15), [(7,6),(7,7),(7,8),(8,5),(8,5),(8,9),(9,6),(9,7),(9,8)]), 
    ...
    ny, nx = dico_patterns[choice][0]
    div = ny//(size-1)
    print(div+2)
    print(ny - (size-2)*div + 2)

    comm = comm0.Split(color=1)
    ...
    if rank == 0:
        ...
        dim_x_last_thread = ny - (size-2)*div + 2
        partial_grid = Grille(dim = (dim_x_last_thread,nx))
    
    else:
        partial_grid = Grille(dim = (div+2,nx))
    ...
    if rank == 0:
        while True:
            ...
            for k in range(1, size):
                cells_new = comm0.recv(source=k)
                if k == 1:
                    grid.cells[0:div,:] = cells_new
                elif k == size -1:
                    grid.cells[(k-1)*div:,:] = cells_new
                else:
                    grid.cells[(k-1)*div:k*div,:] = cells_new   
        ...
        while True:
            cells = comm.recv(source=0)
            partial_grid.cells = cells
            partial_grid.compute_next_iteration()
            comm0.send(partial_grid.cells[1:-1, :], dest=0)
